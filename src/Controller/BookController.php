    ...
    public function Affiche(BookRepository $repository)
    {
        ...
        if ($numPublishedBooks > 0) {
            return $this->render('book/Affiche.html.twig',
             ['publishedBooks' => $publishedBooks, 'numPublishedBooks' 
             => $numPublishedBooks, 'numUnPublishedBooks' => $numUnPublishedBooks]);
...
#[Route('/all', name: 'list_books')]
    public function listBook( BookRepository $repository): Response
    {
        $books = $repository->getBooksByAuthors();
        return $this->render('book/affiche.html.twig', ['books' => $books]);
    }

    #[Route('/getBookByAuthor', name: 'author_getBooks')]
    public function bookcontientm( 
        BookRepository $repo,Request $req): Response
    {
     
      

        return $this->render('book/listBook.html.twig', [
        'books'=>$repo->getBooksByAuthors(),
    ...
    public function searchBook(BookRepository $repo, Request $req){

        $ref = $req->get('ref');
        // dd($ref);
        if($ref != null) {
            $s = $repo->getBookByRef($ref);
            // dd($s);
            return $this->render('book/listBook.html.twig',[
                'books' => $s
            ]);
    ...
    #[Route('/getBookDate', name: 'books_listdate')]
